...
def target_diagnostics(
    ...
    chemical we report:
      * producer_rxns_in_cascade — # cascade reactions that list it as a
        product. (Inflated by cofactor byproducts; not bounded by any cap.)
...
def evaluate_target(
    ...
    """Run traceback (unlimited cutoff) and record cascade diagnostics."""
...
def main() -> None:
    ...
    for shell, chems in sorted(sampled.items()):
        target_results = []
        for chem in chems:
            r = evaluate_target(hg, chem, global_producers)
